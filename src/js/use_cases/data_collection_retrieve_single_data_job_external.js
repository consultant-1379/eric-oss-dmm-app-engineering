...
export default function(access_token) {
    ...
    group('Retrieve single data job', function() {
        if (__ENV.MYVAR == 'DCC'){
        
        group('Retrieve single data job for the given consumer', function() {
            ...
            if (__ENV.GATEWAY == 'SEF'){              
                response_datajob = http.get(DCC_EIC_URL + "/data-access/<api-version>/<consumer-id>/dataJobs/<data-job-id>", JSON.stringify(data), head);
            }
            else if (__ENV.GATEWAY == 'APIGW'){              
                response_datajob = http.get(DCC_GAS_URL + "/data-access/<api-version>/<consumer-id>/dataJobs/<data-job-id>", JSON.stringify(data), head);
            }
